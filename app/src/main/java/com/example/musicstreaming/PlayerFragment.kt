...
class PlayerFragment: Fragment(R.layout.fragment_player) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        initPlayControlBotton(fragmentPlayerBinding)
    ...
    private fun initPlayControlBotton(fragmentPlayerBinding: FragmentPlayerBinding) {
            ...
            player?.addListener(object: Player.EventListener{
                override fun onIsPlayingChanged(isPlaying: Boolean) {
                    ...
                    if(isPlaying){
                        ...
                        binding.ivPlayControl.setImageResource(R.drawable.ic_baseline_play_arrow_24)
                    }
                }
    ...
    private fun updateSeekUi(duration: Long, position: Long){
        binding?.let { binding ->
            ...
            binding.tvPlayTime.text = String.format("%02d:%02d",
                TimeUnit.MINUTES.convert(position, TimeUnit.MILLISECONDS),
                (position / 1000) % 60)
            binding.tvTotalTime.text = String.format("%02d:%02d",
    ...
    private fun initPlayListButton(fragmentPlayerBinding: FragmentPlayerBinding) {
        fragmentPlayerBinding.ivPlaylist.setOnClickListener {
            //todo 만약 서버에서 데이터가 다 불려오지 않을 상태일 때(선택된 음악이 없음)
    ...
    private fun playMusic(musicModel: MusicModel){
        model.updateCurrenPosition(musicModel)
